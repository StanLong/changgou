<template>
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item label="状态">
        <el-select style="width:120px;" v-model="dataForm.status" placeholder="请选择状态" clearable>
          ...
          <el-option label="正在采购" :value="3"></el-option>
          <el-option label="已完成" :value="4"></el-option>
          <el-option label="采购失败" :value="5"></el-option>
        </el-select>
      </el-form-item>
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column type="selection" :selectable="handleSelectable" header-align="center" align="center" width="50"></el-table-column>
      ...
      <el-table-column prop="status" header-align="center" align="center" label="状态">
        <template slot-scope="scope">
          ...
          <el-tag type="info" v-if="scope.row.status==1">待分配</el-tag>
          <el-tag type="info" v-if="scope.row.status==2">已分配</el-tag>
          <el-tag type="wanring" v-if="scope.row.status==3">正在采购</el-tag>
          <el-tag type="success" v-if="scope.row.status==4">已完成</el-tag>
          <el-tag type="danger" v-if="scope.row.status==5">采购失败</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="failReason" header-align="center" align="center" label="失败原因"></el-table-column>
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if="scope.row.status==0" @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
...
export default {
  ...
  methods: {
    handleSelectable(row, index) {
      return row.purchaseId == null ||  row.purchaseId == ""
    },
    mergeItem() {

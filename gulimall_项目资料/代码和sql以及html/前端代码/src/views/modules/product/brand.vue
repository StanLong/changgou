<template>
  <div class="mod-config">
    ...
    <el-dialog title="关联分类" :visible.sync="cateRelationDialogVisible" @closed="catelogPath = []" width="50%">
...
export default {
  ...
  methods: {
    ...
    // 删除
    deleteHandle(data) {
      console.log("deleteHanler: ", data)
      
      var ids = data ? [data.brandId] : this.dataListSelections.map(item => {
            return item.brandId;
          });
      var nameList = data ? [data.name] : this.dataListSelections.map(item => {
            return item.name;
          });
      this.$confirm(
        `确定对【${nameList.join(",")}】品牌进行【${data ? "删除" : "批量删除"}】操作?`,
        "提示",
        {

...
<template>
  <el-row :gutter="20">
    ...
    <el-col :span="6"
      ><category-tree
        @category-tree-node-click="handleTreeNodeClick"
      ></category-tree
    ></el-col>
    <el-col :span="18">
...
export default {
  data() {
    return {
      ...
      // 向后台请求属性分组列表时，如果catId是0表示取全部，否则取指定catId对应的分类下的属性组
      catId: 0,
    };
  },
  components: {
  ...
  methods: {
    ...
    deleteHandle(data) {
      ...
      var nameList = data
        ? [data.attrGroupName]
        : this.dataListSelections.map((item) => {
            return item.attrGroupName;
          });
      this.$confirm(
        `确定对【${nameList.join(",")}】属性组进行【${
          data ? "删除" : "批量删除"
        }】操作?`,
        "提示",
        {
    ...
    handleTreeNodeClick(data, node, component) {
      console.log(
        "父组件attrgroup接收到子组件category-tree传来的事件",
        data,
        node,
        component
      );
      // 除非惦记的是第三级分类，才去查询此分类下的所有属性组
      if (node.level == 3) {
        // 获取指定分类下的属性组集合
